...
let [title1, setTitle] = useState('Community Borehole')
...
useEffect(()=>{
 ...
 //       alert(JSON.parse(val)[0].title)
   ...
   await   setUid(val)
    ...
    AsyncStorage.getItem('mon').then(val=>{
        // alert(JSON.parse(val)[0].title)
       //  const projectsVal = JSON.parse(val)
       setmon(val)
     })
...
const  GotoReport=(e,title,community,lga,company,ward,lot,statesup,lgsup,phase)=>{
   // return alert(lot)
    //    alert(this.state.mid)
    navigation.navigate('GenReport',{uid,pid:e,title,community,lga,mon,contractor:company,ward,lot,phase,lgsup,statesup})
}
...
Object.keys(projects).map((e,i)=>{
    if(projects[e].lga===lga && projects[e].title===route.params.title){
   // if(projects[e].phase==='6d'){

row.push(<TouchableOpacity key={i} style={styles.row} onPress={()=> login==='granted1'? GotoReport(projects[e].id,
projects[e].title,projects[e].community,projects[e].lga,projects[e].company,projects[e].ward,
projects[e].lot, projects[e].first_name+' '+projects[e].last_name,projects[e].fn+' '+projects[e].ln,
projects[e].phase) : alert('you must sign in to give feedback')}>

            <Text >
                <Text style={styles.txtl}>{projects[e].community}<Text style={styles.sep}>|</Text>                
            </Text>
            ...
            <Text  style={styles.txtphase} > {projects[e].company} </Text>
            <Text  style={styles.txtphase} > {projects[e].status} </Text>
    ...
    return(
        ...
        <Picker style={{borderWidth:4,borderColor:'gray', height: 50, width: 150}}
                 selectedValue = {lga}  onValueChange = {handleLga}>
{lgas.map(e=>
